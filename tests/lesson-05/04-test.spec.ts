...
test("Bai 4: Personal notes", async ({ page }) => {
    ...
    await test.step("Add 10 notes", async () => {
        ...
        const btnInput = page.locator("//button[@id = 'add-note']");

        for (let i = 0; i < notes.length; i++) {
            await titleInput.fill(notes[i].title);
            await contentInput.fill(notes[i].content);
    ...
    await test.step("Search", async() => {
        // search với từ khoá "một hoặc nhiều"
